...
import { Tab, Tabs, Container, Alert, Spinner, Button } from "react-bootstrap";

// Import components
import NGODonations from "./NGODonations";
import ProfileTab from "./ProfileTab";
import CreateFundReport from "../transparency/CreateFundReport";
    ...
    const checkAuth = async () => {
      try {
        ...
        if (userData.role !== "NGO" && userData.role !== "ADMIN") {
          setError(
            "Access denied. This dashboard is for NGO users and administrators only."
          );
          setLoading(false);
          return;
        }
    ...
    try {
      ...
      if (ngoResponse.status === 200 && ngoResponse.data) {
        ...
        const profileData = {
          ...
          causes: Array.isArray(ngoData.causes)
            ? ngoData.causes
            : ngoData.cause
            ? [ngoData.cause]
            : [],
        ...
        if (storedUser) {
          const fallbackData = {
            ...
            causes: Array.isArray(storedUser.causes)
              ? storedUser.causes
              : storedUser.cause
              ? [storedUser.cause]
              : [],
  ...
  return (
    <Container fluid className="py-4">
      ...
      >
        ...
        <Tab
          eventKey="volunteer-needs"
          title={
            <span>
              <FaHandsHelping className="me-1" /> Volunteer Needs
            </span>
          }
        >
