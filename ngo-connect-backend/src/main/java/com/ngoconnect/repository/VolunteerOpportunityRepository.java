...
public interface VolunteerOpportunityRepository extends JpaRepository<VolunteerOpportunity, Long> {
    ...
    List<VolunteerOpportunity> findByCauseAndLocation(@Param("cause") String cause, @Param("location") String location);
    ...
    List<VolunteerOpportunity> findByCauseAndLocationAndUrgency(@Param("cause") String cause, @Param("location") String location, @Param("urgency") UrgencyLevel urgency);
    
    @Query("SELECT v FROM VolunteerOpportunity v WHERE v.cause = :cause AND v.workType = :workType")
    List<VolunteerOpportunity> findByCauseAndWorkType(@Param("cause") String cause, @Param("workType") String workType);
    
    @Query("SELECT v FROM VolunteerOpportunity v WHERE v.startDate >= :startDate AND v.endDate <= :endDate")
    List<VolunteerOpportunity> findByDateRange(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
